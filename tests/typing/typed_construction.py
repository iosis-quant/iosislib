...
from iosislib.core.utils import numpy_to_series, series_to_numpy, series_to_torch, torch_to_series
...
numpy_values = series_to_numpy(series)
torch_values = series_to_torch(series, allow_copy=True)
...
assert_type(numpy_to_series("probability", np.array([0.25])), pl.Series)
assert_type(torch_values, torch.Tensor)
assert_type(torch_to_series("probability", torch_values), pl.Series)
