...
def test_l1_blocks_buy_that_would_breach() -> None:
    ...
    assert result.get_column("order").to_list() == [[50.0], [0.0]]
    assert result.get_column("cash").to_list() == [10.0 - 25.0, 10.0 - 25.0]
...
def test_l1_zero_equity_boundary_triggers_liquidation() -> None:
    ...
    result = run_l1(frame, 1, 10.0)
    assert result.get_column("order").to_list() == [[100.0], [-100.0]]
    assert result.get_column("cash").to_list() == [10.0 - 50.0, 0.0]
    assert result.get_column("balance").to_list() == [[100.0], [0.0]]
...
def test_l1_liquidates_on_ruin_then_freezes() -> None:
    ...
    assert result.get_column("order").to_list() == [[10.0], [-10.0], [0.0]]
    assert result.get_column("proposed_order").to_list() == [[10.0], [5.0], [0.0]]
    assert result.get_column("balance").to_list() == [[10.0], [0.0], [0.0]]
    assert result.get_column("cash").to_list() == [-9.0, -8.1, -8.1]
...
def test_l1_wide_liquidates_longs_and_shorts() -> None:
    ...
    assert result.get_column("order").to_list()[1] == [-10.0, 10.0] + [0.0] * 6
    assert result.get_column("order").to_list()[2] == [0.0] * width
    assert result.get_column("balance").to_list()[1] == [0.0] * width
    assert result.get_column("cash").to_list()[1] == pytest.approx(0.1 - 0.2)
    assert result.get_column("proposed_order").to_list()[2] == [0.0] * width
...
def test_l1_ruin_with_flat_book_early_returns() -> None:
    ...
    assert result.get_column("proposed_order").to_list() == [[5.0], [0.0]]
    assert result.get_column("cash").to_list() == [0.0, 0.0]
...
def test_l1_wide_gates_assets_in_order() -> None:
    ...
    assert result.get_column("order").to_list() == [[101.0] + [0.0] * 7]
    assert result.get_column("cash").to_list() == [10.5 - 50.5]
    assert result.get_column("equity").to_list()[-1] >= 0.0
...
def test_l2_blocks_fill_on_slippage_breach() -> None:
    ...
    assert result.get_column("unfilled").to_list() == [[10.0]]
    assert result.get_column("cash").to_list() == [1.0]
...
def test_l2_liquidates_on_ruin_then_freezes() -> None:
    ...
    result = run_l2(frame, 1, 1.0)
    assert result.get_column("order").to_list() == [[10.0], [-10.0], [0.0]]
    assert result.get_column("proposed_order").to_list() == [[10.0], [3.0], [0.0]]
    assert result.get_column("unfilled").to_list() == [[0.0], [0.0], [0.0]]
    assert result.get_column("fill_price").to_list() == [[0.5], [0.01], [0.0]]
    assert result.get_column("balance").to_list() == [[10.0], [0.0], [0.0]]
    assert result.get_column("cash").to_list() == [-4.0, -3.9, -3.9]
...
def test_l2_wide_liquidates_through_dispatcher() -> None:
    ...
    assert result.get_column("order").to_list()[1][0] == -10.0
    assert result.get_column("order").to_list()[2] == [0.0] * width
    assert result.get_column("proposed_order").to_list()[2] == [0.0] * width
    assert result.get_column("balance").to_list()[1] == [0.0] * width
    assert result.get_column("cash").to_list()[1:] == [-3.9, -3.9]
...
def test_l2_wide_voids_breaching_asset_only() -> None:
    ...
    assert result.get_column("order").to_list() == [[0.0, 1.0] + [0.0] * 6]
    assert result.get_column("unfilled").to_list()[0][0] == 100.0
    assert result.get_column("cash").to_list() == [4.5]
    assert result.get_column("equity").to_list()[-1] >= 0.0
